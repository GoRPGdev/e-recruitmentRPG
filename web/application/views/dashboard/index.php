		...
		<div class="dash-card" style="margin-bottom:22px; padding:18px 20px">
			<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:14px; border-bottom:1px solid var(--border); padding-bottom:10px">
				<div style="display:flex; align-items:center; gap:10px">
					...
					<h2 style="font-size:15px; font-weight:700; margin:0; color:var(--text)">Kandidat Tertahan (>7 Hari)</h2>
					<span class="tag <?= count($d['aging']) > 0 ? 'off' : 'on' ?>" style="font-size:11px">
						<?= count($d['aging']) ?> Kandidat Tertahan
					</span>
				</div>
				<span class="faint" style="font-size:12px">Kandidat yang berada lebih dari 7 hari di tahap seleksi aktif</span>
			</div>

			<?php if ( ! $d['aging']): ?>
				<div style="padding:24px 16px; text-align:center; background:var(--surface-2); border-radius:8px">
					<p class="muted" style="margin:0; font-size:13px">&#10003; Semua proses seleksi kandidat berjalan lancar tanpa ada yang tertahan lama.</p>
				</div>
			<?php else: ?>
				<div class="table-responsive-fit" style="overflow-x:auto">
					<table class="dash-table">
