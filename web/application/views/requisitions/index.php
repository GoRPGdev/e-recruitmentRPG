...
<div class="container" style="padding-top:20px; padding-bottom:40px">
	<div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:20px; flex-wrap:wrap; gap:12px">
		<div>
			<div class="eyebrow" style="margin-bottom:4px">Manajemen Rekrutmen</div>
			<h1 style="margin:0; font-size:24px; font-weight:700">Permintaan Tenaga Kerja (MPR)</h1>
			<p class="muted" style="margin:4px 0 0; font-size:13px">Daftar pengajuan kebutuhan personel dari tiap departemen &amp; outlet.</p>
		</div>
		<div>
			<?php if (has_permission('BUAT_MPR')): ?>
				<a class="btn btn-primary" href="<?= site_url('requisitions/create') ?>" style="display:inline-flex; align-items:center; gap:6px">
					...
					<span>Buat Permintaan Baru (MPR)</span>
				</a>
			<?php endif; ?>
		</div>
	</div>
	...
	<div style="margin-bottom:24px">
		...
		<div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(200px, 1fr)); gap:14px; margin-bottom:16px">
			...
			<div class="card" style="padding:14px 16px; display:flex; align-items:center; gap:12px; margin:0">
				...
				<div>
					<div class="muted" style="font-size:11.5px; font-weight:600">Aktif &amp; On Progress</div>
						...
						<tbody>
							...
							<tr class="mpr-row" data-href="<?= site_url('requisitions/view/' . (int) $r['id_req']) ?>" style="cursor:pointer">
								...
								<td style="padding:12px 14px; text-align:center">
									...
									<?php elseif ($r['status_req'] === 'Revisi_HR' && !empty($r['catatan_hr'])): ?>
										<div style="font-size:11px; color:var(--warn); margin-top:4px; max-width:180px; margin-inline:auto; line-height:1.3; word-break:break-word" title="Arahan Revisi HR: <?= html_escape($r['catatan_hr']) ?>">
											<strong>Revisi HR:</strong> <?= html_escape(mb_strimwidth($r['catatan_hr'], 0, 45, '...')) ?>
										</div>
									<?php elseif ($r['status_req'] === 'Revisi_BOD' && !empty($r['catatan_bod'])): ?>
										<div style="font-size:11px; color:var(--warn); margin-top:4px; max-width:180px; margin-inline:auto; line-height:1.3; word-break:break-word" title="Arahan Revisi BOD: <?= html_escape($r['catatan_bod']) ?>">
											<strong>Revisi BOD:</strong> <?= html_escape(mb_strimwidth($r['catatan_bod'], 0, 45, '...')) ?>
										</div>
									<?php endif; ?>
								</td>
...
document.addEventListener('DOMContentLoaded', function() {
	document.querySelectorAll('.mpr-row').forEach(function(row) {
		row.addEventListener('click', function(e) {
			if (e.target.closest('button, a, input, select, .mpr-dropdown, .mpr-menu-btn')) {
				return;
			}
