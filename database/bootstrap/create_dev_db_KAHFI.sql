...
/* 1. Database kosong ---------------------------------------------------------- */
IF DB_ID('RPG_EREC_DEV_KAHFI') IS NULL
    CREATE DATABASE RPG_EREC_DEV_KAHFI;
GO

/* 2. Kunci ke perilaku SQL Server 2008 R2 + recovery ringan (DB dev) --------- */
ALTER DATABASE RPG_EREC_DEV_KAHFI SET COMPATIBILITY_LEVEL = 100;
ALTER DATABASE RPG_EREC_DEV_KAHFI SET RECOVERY SIMPLE;
GO

/* 3. Login aplikasi -----------------------------------------------------------
   Dilewati kalau sudah ada (instance dipakai bareng dengan RPG_EREC_DEV_KIKI). */
IF NOT EXISTS (SELECT 1 FROM sys.server_principals WHERE name = 'erec_app')
    CREATE LOGIN erec_app
        WITH PASSWORD        = 'MASUKKAN_PASSWORD_KUAT_DI_SINI',
             CHECK_POLICY    = OFF,
             DEFAULT_DATABASE = RPG_EREC_DEV_KAHFI;
GO

/* 4. User + hak DDL, HANYA di DB dev ini ------------------------------------- */
USE RPG_EREC_DEV_KAHFI;
GO
