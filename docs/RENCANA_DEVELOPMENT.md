...
- [x] RBAC tiga tingkat: umum / dokumen identitas / finansial — mekanisme + matriks + checklist uji per peran ✅ (`docs/MATRIKS_HAK_AKSES.md`, helper `gate_sensitif`/`require_any`/`require_all`); seluruh gap G1 (`Requisitions::approve`), G2 (`range_gaji`), G3 (`Candidates::detail` layar detail kandidat, data kesehatan UU PDP, gaji & rekening), G4b (`scoping USER_DEPT`), G6 (`BUAT_MPR`), G7 (`Flowbuilder::flow_docs`) telah diselesaikan dan teruji 100%
- [x] `ACCESS_LOG_SENSITIF` — buka dokumen IDENTITAS/FINANSIAL ✅ (`gate_sensitif`), export kolom gaji/rekening ✅, view MPR & form offer ber-gate GAJI tercatat ke log, layar detail kandidat mencatat log KESEHATAN, GAJI, dan FINANSIAL saat dibuka oleh yang berhak
...
- [x] Matriks hak akses diuji per peran, satu per satu — probe otomatis 6 peran ✅ (`tools/rbac-probe.sh`, hasil di `docs/MATRIKS_HAK_AKSES.md §3b`); G1, G2, G3, G4, G4b, G5, G6, G7 terverifikasi tuntas
