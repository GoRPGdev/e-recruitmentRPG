...
| **G4b** | Daftar **kandidat & CV** belum di-scope "req sendiri" untuk USER_DEPT. | USER_DEPT lihat kandidat dept lain | **Kahfi** (`Requisition_model`), **Kiki** (`Dashboard_model`/`sp_Dashboard`) + 1 migrasi | **Keputusan HR 2026-09-04: USER_DEPT hanya lihat kandidat dari MPR yang `id_departemen` = departemen dia** (via `id_posisi → M_POSISI.id_departemen`), bukan per pemohon. BOD tidak di-scope (HR: "lihat aja"). **Prasyarat:** `M_USERS` belum simpan `id_departemen` (cuma `departemen_snapshot` teks). Perlu migrasi tambah `M_USERS.id_departemen` (FK, NULL untuk peran non-dept) + `sp_Login`/`sp_GetUserPermissions` kembalikan nilainya ke sesi. Baru filter di list MPR/kandidat/pipeline (Kahfi) + funnel/metrik dashboard (Kiki). |
